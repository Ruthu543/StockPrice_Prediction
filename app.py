...
st.subheader('Original Close Price with MA 100 and 250')
...
# ---------------- STORE SUMMARY ----------------
if st.button("Save Summary to MongoDB"):
    avg_prediction = ploting_data["predictions"].mean()
    avg_actual = ploting_data["original_test_data"].mean()

    summary = {
        "stock": stock,
        "avg_prediction": float(avg_prediction),
        "avg_original": float(avg_actual),
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    }

    collection.insert_one(summary)
    st.success("Summary saved to MongoDB successfully!")
